...
from random import choice     # random.choice(sequence) - случайный элемент непустой последовательности.#
#S.lower()	ѕреобразование строки к нижнему регистру
...
def mutate(member):
  ...
  max_index = len(input_str)                #WE WILL ADD THE PROBABILITY OF MUTATION HERE
...
def reproduce(member, k):             #HERE WE CREATE NEW GENERATION! WE NEED TO CHANGE IT TO CROSSINGOVER
  output = []
  for i in range(0,k):
    output.append(mutate(member))
  return output

def select(offsprings, selection, size):                   #SELECTION FIRST SIZE BEST OFFSPINGS
